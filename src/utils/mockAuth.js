// Simulated auth for local development without a backend.
// Production and deployed builds use auth.js and main.js instead.
...
export async function deleteArticle(articleUrl) {
  const saved = getSavedFromStorage().filter((item) => item.url !== articleUrl);
  saveToStorage(saved);
  return delay(saved);
}
